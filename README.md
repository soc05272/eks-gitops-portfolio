...
지금 제 일은 통합관제 솔루션 ZENIUS를 고객사 인프라에 구축하고 기술지원하는 것입니다. 장애가 발생하면
로그를 분석해 원인을 찾아내지만, 그 인프라가 왜 그렇게 설계되었는지까지는 알 수 없는 위치였습니다.
설계는 제 몫이 아니었기 때문입니다. 이전 직장인 오투에서 NHN·Naver Cloud의 AI 학습 환경을 운영하며
Docker 이미지로 표준화해 제공 시간을 40% 줄이고 5TB 넘는 데이터를 무중단 이관했을 때도 마찬가지였습니다.
효과는 분명했지만, 그 아래의 클라우드는 여전히 다른 사람이 설계한 것이었습니다.
