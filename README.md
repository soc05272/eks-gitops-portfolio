...
6. [진행 현황](#6-진행-현황)
7. [비용 전략](#7-비용-전략)
8. [트러블슈팅과 의사결정](#8-트러블슈팅과-의사결정)

---

## 1. 프로젝트를 시작한 계기

지금 다니는 회사에서 저는 통합관제 솔루션(ZENIUS)을 고객사 인프라에 구축하는 일을 합니다.
공공기관과 기업 고객의 환경에 관제 시스템을 올리고, 서버 수십 대의 지표가 대시보드에
잡히게 만들고, 장애가 나면 로그를 뒤져 원인을 찾습니다. 그런데 이 일을 하다 보면 묘한
지점이 있습니다. 화면에 그려지는 인프라가 전부 남이 설계한 것이라는 점입니다. 왜 서브넷이
이렇게 나뉘었는지, 왜 이 구간만 이중화가 빠져 있는지 — 궁금해도 제가 결정한 게 아니라서,
답은 늘 다른 사람에게 있었습니다.

전 직장에서 클라우드 운영을 맡았을 때도 비슷했습니다. AI 학습 환경을 매번 수동으로
세팅하는 게 답답해서 Docker 이미지로 묶었더니 제공 시간이 40% 줄었는데, 그때 처음으로
"운영은 자동화로 바꿀 수 있구나"를 몸으로 느꼈습니다. 다만 그것도 남이 만들어 둔 클라우드
위에서의 개선이었지, 그 아래가 어떻게 생겼는지는 여전히 모르는 채였습니다.

작년에 AWS SA Professional을 취득했습니다. 그런데 합격하고 나니 오히려 마음이 불편해졌습니다.
시험에서는 VPC를 설계할 줄 아는데, 정작 제 계정에는 VPC가 하나도 없었기 때문입니다.
머리로 아는 것과 손으로 세워 본 것 사이의 간격이 얼마나 되는지, 직접 확인해 보고 싶었습니다.

그래서 이 프로젝트를 시작했습니다. 조건은 하나였습니다 — 설계부터 장애 대응, 비용,
마지막 회수까지 전부 제 책임일 것. 3주 동안 실제로 장애를 만났고(7건 기록), 선택의 이유를
문서로 남겼고(ADR 3건), 같은 환경을 다섯 번 부수고 다시 세웠습니다. 관제 화면 너머에서
궁금하기만 했던 것들을, 지금은 제가 만든 인프라에서 직접 확인하고 있습니다.
