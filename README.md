...
지금 회사에서 저는 통합관제 솔루션(ZENIUS)을 고객사 인프라에 구축하는 일을 합니다.
장애가 나면 로그를 뒤져 원인을 찾지만, 화면에 그려지는 인프라는 전부 남이 설계한 것이라
"왜 이렇게 만들었는지"에 대한 답은 늘 다른 사람에게 있었습니다. 전 직장에서 Docker
이미지로 AI 학습 환경 제공 시간을 40% 줄여봤을 때도 마찬가지였습니다 — 자동화의 효과는
몸으로 느꼈지만, 그 아래의 클라우드가 어떻게 생겼는지는 여전히 모르는 채였습니다.

작년에 AWS SA Professional을 취득하고 나니 오히려 마음이 불편해졌습니다. 시험에서는
VPC를 설계할 줄 아는데, 정작 제 계정에는 VPC가 하나도 없었기 때문입니다. 그래서
설계부터 장애 대응, 비용, 회수까지 전부 제 책임인 프로젝트를 시작했습니다. 3주 동안
장애 7건을 기록하고, 선택의 이유를 ADR로 남기고, 같은 환경을 다섯 번 부수고 다시
세웠습니다. 관제 화면 너머에서 궁금하기만 했던 것들을, 지금은 제가 만든 인프라에서
직접 확인하고 있습니다.
